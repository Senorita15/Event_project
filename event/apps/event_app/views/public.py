...
class reservation_creation_form(View):
    def get(self, request, pk=None):
        evenement = get_object_or_404(
            Event, pk=pk
        )  # recuperation de l'objet evenement pour la vue
        serializer = Reservation_public_serializer()
        context = {"serializer": serializer, "evenement": evenement}
...
class ReservationViewSet_public(viewsets.ViewSet):
    ...
    evenement_detail = (
        "event_app/public/add.html"  # page de creation de vue d'un evenement
    )
    ...
    def retrieve(self, request, pk=None):
        ...
        serializer = (
            Reservation_public_serializer()
        )  # recuperation du serializer pour affichage du formulaire
        # de reservation d'un ticket
        context = {"evenement": evenement, "serializer": serializer}
