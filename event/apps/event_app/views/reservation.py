...
class FilterReservationDataPage(View):
    def get(self, request):
        # Filter first names
        first_name_list = Reservation.objects.values_list("first_name", flat=True).distinct()
        # Filter last names
        last_name_list = Reservation.objects.values_list("last_name", flat=True).distinct()
        # Filter phone numbers
        phone_number_list = Reservation.objects.values_list("phone_number", flat=True).distinct()
...
class ReservationViewSet(viewsets.ViewSet):
    ...
    def list(self,request):
        if request.user.is_authenticated and request.user.role == "gerant":
            queryset = Reservation.objects.filter(ticket_event__event__created_by=request.user)
            if queryset:
    ...
    def create_ticket(self, request, pk=None):
        ...
        capacite =  event.limit_attendees #recuperation de la valeur du nombre total de participant prevu
        if not event:
            context = {
                        'message': "NO EVENT CHOSEN",     
                        }
            return Response(context, status=status.HTTP_400_BAD_REQUEST) 
        serializer = Reservation_public_serializer(data=request.data) #recuperation des donnees du formulaire
        if serializer.is_valid():
            # Récupérer les données valides du serializers
            data = serializer.validated_data
            # Récupérer les numéros de place disponible
            numeros_places_existants = Reservation.objects.filter(ticket_event__event=event).values_list('ticket_number', flat=True)
            if numeros_places_existants.exists():
                    # Affectation d'un numero de place parmi les places restantes
                    num_place_disponible = next((i for i in range(1, capacite+1) if i not in numeros_places_existants), None)
